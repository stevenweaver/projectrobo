...
void loop() {
    ...
    Input_RIGHT= clicks_RIGHT;
    Input_LEFT = clicks_LEFT;
    wait_time =  millis() - StartTime;
    
   if(wait_time < TIME_LIMIT_1 )  { // check if the metro has passed it's interval .
     PID_RIGHT.SetOutputLimits(PID_RIGHT_OUTPUT_MIN,PID_RIGHT_OUTPUT_MAX - 70);
      PID_LEFT.SetOutputLimits(PID_LEFT_OUTPUT_MIN,PID_LEFT_OUTPUT_MAX - 63);
...
//    }
   ...
   //   Serial.println(clicks_RIGHT);
    ...
    //digitalWrite(MOTOR_LEFT_CONTROL1, HIGH);
    analogWrite(Motor_Driver.pwm_pin_left, 150);
...
void sendSerialInfo(int left_us_val, int left_flex_val,int right_us_val, int right_flex_val,int compass_val, int pos, int clicks_RIGHT, int clicks_LEFT)
{
    ...
    sprintf(xml,"<?xml version=\"1.0\"?><sensor><c>%d</c><f><l>%d</l><r>%d</r></f><us><l>%d</l><r>%d</r></us><b>%d</b><we><a>%d</a><b>%d</b></we><done>%d</done></sensor>", compass_val, left_flex_val, right_flex_val, left_us_val, right_us_val, pos, clicks_RIGHT, clicks_LEFT, done); 
    Serial.println(xml);
    return;
}
