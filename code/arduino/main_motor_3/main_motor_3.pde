...
void sendSerialInfo( float clicks_RIGHT, float clicks_LEFT, int done_RIGHT, int done_LEFT)
{
    ...
    int clicks_RIGHT_XML = (clicks_RIGHT /10);
     int clicks_LEFT_XML = (clicks_LEFT /10);
     

    sprintf(xml,"<?xml version=\"1.0\"?><motor><cl><r>%d0</r><l>%d0</l></cl><d><r>%d</r><l>%d</l></d></motor>", clicks_RIGHT_XML, clicks_LEFT_XML, done_RIGHT, done_LEFT); 
    Serial.println(xml);
    return;
}
...
//      
 ...
 if(Serial_Setpoint_Updated == 1 && Serial_Direction_Updated == 1)
 
      { Serial_Setpoint_Updated = 0;
        Serial_Direction_Updated = 0;
        done_RIGHT = 0;
        done_LEFT = 0;
        MsTimer2::start(); 
      }
